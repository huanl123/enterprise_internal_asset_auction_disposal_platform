  ...
  <div class="my-transactions">
    <el-card>
      ...
      <el-table v-loading="loading" :data="transactionList" stripe border>
        <el-table-column prop="code" label="交易单号" width="180" />
        ...
        <el-table-column prop="paymentStatus" label="付款状态" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="getPaymentTag(row.paymentStatus)">{{ getPaymentText(row.paymentStatus) }}</el-tag>
          </template>
        </el-table-column>
...
const filterLabel = computed(() => {
  ...
  if (queryForm.value.paymentStatus) {
    parts.push(`付款状态：${getPaymentText(queryForm.value.paymentStatus)}`)
  }
  return parts.join('，')
})
...
const getPaymentText = (status) => {
  const value = String(status || '').trim().toLowerCase()
  if (value === 'pending') return '待财务确认收款'
