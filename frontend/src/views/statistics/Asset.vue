  ...
  <div class="asset-statistics">
    <el-card>
      ...
      <el-row :gutter="20" style="margin-top: 20px">
        ...
        <el-col :xs="24" :md="12">
          <el-card>
            <template #header>{{ deptCompareTitle }}</template>
...
const isScopedDepartmentView = computed(() => !showDepartmentFilter.value)
...
const trendTitle = computed(() => {
  if (trendPeriod.value === 'day') {
    if (isScopedDepartmentView.value) {
      return selectedMonth.value
        ? `${scopedDepartmentName.value}成交趋势（${selectedMonth.value}·按天）`
        : `${scopedDepartmentName.value}成交趋势（点击月份查看按天）`
    }
    return selectedMonth.value ? `成交趋势（${selectedMonth.value}·按天）` : '成交趋势（点击月份查看按天）'
  }
  return isScopedDepartmentView.value
    ? `${scopedDepartmentName.value}拍卖成交额月度趋势（近6个月）`
    : '拍卖成交额月度趋势（近6个月）'
})

const deptCompareTitle = computed(() => {
  return isScopedDepartmentView.value
    ? `拍卖结果数量对比（成交/流拍） · ${scopedDepartmentName.value}`
    : '各部门拍卖结果数量对比（成交/流拍）'
})
...
const handleExport = () => {
  ...
  const csvContent = [headers, ...rows]
    .map(row => row.map(value => `"${String(normalizeCsvValue(value))}"`).join(','))
    .join('\n')
