...
$F_CH4_DETAIL = "LHvbVuB6IAAgAPt8337mi8Z+vouhiw5Onluwcw=="
...
try {
  ...
  # Remove any stray "old chapter 4 detailed design" text embedded in other headings.
  Replace-All $doc (S $F_CH4_DETAIL) ""

  # Insert Chapter 4 + Chapter 5 skeleton right before the first 4.1 module section.
  $iImplStart = FindFirstRegex $doc '^4\.1\b'
  if ($iImplStart -eq 0) { throw 'Cannot find 4.1 module section.' }

  $insertPos = $doc.Paragraphs.Item($iImplStart).Range.Start

  # Insert in reverse order because Paragraphs.Add inserts *before* the given range.
  # Desired order: 3.5 -> Chapter 4 -> Chapter 5 -> existing content.

  # Chapter 5 (implementation) heading and 5.1/5.2.
  $pCh5 = InsertPara $doc $insertPos (S $T_CH5) 1
  $p51 = InsertPara $doc $pCh5.Range.End (S $T_51) 2
  [void](InsertPara $doc $p51.Range.End (S $T_52) 2)

  # Chapter 4 (design) skeleton.
  $pCh4 = InsertPara $doc $insertPos (S $T_CH4) 1
  $p41 = InsertPara $doc $pCh4.Range.End (S $T_41) 2
  $p411 = InsertPara $doc $p41.Range.End (S $T_411) 3
  $p412 = InsertPara $doc $p411.Range.End (S $T_412) 3
  $p42 = InsertPara $doc $p412.Range.End (S $T_42) 2
  $p43 = InsertPara $doc $p42.Range.End (S $T_43) 2
  [void](InsertPara $doc $p43.Range.End (S $T_44) 2)

  # Ensure Chapter 3 ends with 3.5.
  [void](InsertPara $doc $insertPos (S $T_35) 2)

  # Renumber module headings 4.1..4.6 -> 5.2.1..5.2.6 up to the first occurrence of the original Chapter 5 (tests).
  $p5prefix = (S $T_CH5).Substring(0,3)
  $p6prefix = (S $T_CH6).Substring(0,3)
  $iCh5Tests = FindNthStartsWith $doc $p5prefix 2
  if ($iCh5Tests -eq 0) { $iCh5Tests = FindFirstStartsWith $doc $p6prefix; if ($iCh5Tests -eq 0) { $iCh5Tests = $doc.Paragraphs.Count + 1 } }
  ...
  for ($i = $iImplStart; $i -lt $iCh5Tests; $i++) {
  ...
  # Rename original tests chapter (second "第五章") to Chapter 6.
  $iCh5Tests = FindNthStartsWith $doc $p5prefix 2
  if ($iCh5Tests -ne 0) { $pp = $doc.Paragraphs.Item($iCh5Tests); $pp.Range.Text = (S $T_CH6); SetHeading $pp 1 }

  # Renumber tests headings 5.x -> 6.x and demote extra 5.11+ headings.
  ...
  # Convert the last "第六章 ..." (conclusion) to "6.3  结论".
  $iCh6Conc = FindLastStartsWith $doc $p6prefix
